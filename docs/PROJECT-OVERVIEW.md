...
>
> **Перечни ниже — тоже снимок той эпохи, а не описание отгруженного продукта:** имена MCP-тулов,
> gRPC-методов и REST-маршрутов относятся к кастодиальной сборке (`preview_send`/`execute_send`
> в нынешней линии не существуют вовсе). Числа и списки здесь не правятся под сегодняшний день —
> это переписало бы историю; актуальную поверхность смотри в `PROJECT-STATUS.md`.
